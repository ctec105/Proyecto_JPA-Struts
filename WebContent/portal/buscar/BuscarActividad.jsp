		...
		function enviaDatos() {
			try {
				...
				if (document.forms[0].cod.length == undefined) {
					opener.document.forms[0].buscAct.value = document.forms[0].cod.value;
				} else {
					for (i = 0; i < document.forms[0].cod.length; i++) {
						if (document.forms[0].cod[i].checked)
							break;
					}
					opener.document.forms[0].buscAct.value = document.forms[0].cod[i].value;
				}
				window.close();
			} catch (e) {
				alert(e.value);
				...
				<div class="Post-inner" style="height: 330px">
					<s:form theme="simple">
						...
						<table>
							<tr>
								<td>Nombre:</td>
								<td><s:textfield name="actividad.descripcion" /></td>
							</tr>
							<tr>
								<td colspan="2" align="center">
									<s:submit action="popBuscarActividad" value="Buscar" />
								</td>
							</tr>
						</table>
						<dt:table name="actividades" id="bean" class="simple" pagesize="5" requestURI="popBuscarActividad">
