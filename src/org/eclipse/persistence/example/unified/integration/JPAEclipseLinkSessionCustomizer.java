	...
	public void customize(Session session) throws Exception {
		...
		try {
			...
			if (null != context) {
				...
				// and may still result in the COMPOSITE_NAME_LOOKUP being set
				...
				// persistence.xml
				// then set both connectors to be safe
				...
				System.out
						.println("_JPAEclipseLinkSessionCustomizer: configured "
								+ connector.getName());
			} else {
				throw new Exception(
						"_JPAEclipseLinkSessionCustomizer: Context is null");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
